...
// (tokens wrapped in hsl(...) to match this project's Tailwind v3 channels)
// -------------------------------------------------------------------------
const STYLES = `
.cinematic-footer-wrapper {
  font-family: var(--font-space-grotesk), 'Plus Jakarta Sans', sans-serif;
...
@keyframes footer-heartbeat {
  0%, 100% { transform: scale(1); filter: drop-shadow(0 0 5px color-mix(in oklch, hsl(var(--primary)) 50%, transparent)); }
  15%, 45% { transform: scale(1.2); filter: drop-shadow(0 0 10px color-mix(in oklch, hsl(var(--primary)) 80%, transparent)); }
...
.footer-aurora {
  background: radial-gradient(
    circle at 50% 50%,
    color-mix(in oklch, hsl(var(--primary)) 18%, transparent) 0%,
    color-mix(in oklch, hsl(var(--primary)) 6%, transparent) 40%,
    transparent 70%
  );
}
.footer-glass-pill {
...
.footer-giant-bg-text {
  ...
  color: transparent;
  -webkit-text-stroke: 1px color-mix(in oklch, hsl(var(--foreground)) 6%, transparent);
  background: linear-gradient(180deg, color-mix(in oklch, hsl(var(--foreground)) 12%, transparent) 0%, transparent 60%);
...
const MarqueeItem = () => (
  <div className="flex items-center space-x-12 px-6">
    ...
    <span>RAG &amp; Agentic AI</span> <span className="text-neutral-400/60">✦</span>
    <span>Live Deployments</span> <span className="text-live/60">✦</span>
    <span>Open Source</span> <span className="text-neutral-400/60">✦</span>
    ...
    <>
      ...
      >
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-background text-foreground cinematic-footer-wrapper">
          ...
          <div className="absolute top-12 left-0 w-full overflow-hidden border-y border-white/10 bg-ink/60 backdrop-blur-md py-4 z-10 -rotate-2 scale-110 shadow-2xl">
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 mt-20 w-full max-w-5xl mx-auto">
            ...
            <div ref={linksRef} className="flex flex-col items-center gap-6 w-full">
              ...
              <div className="flex flex-wrap justify-center gap-3 md:gap-6 w-full mt-2">
                <MagneticButton as="a" href="#client" className="footer-glass-pill px-6 py-3 rounded-full text-muted-foreground font-medium text-xs md:text-sm hover:text-foreground">
                  Client Work
                </MagneticButton>
                <MagneticButton as="a" href="#ai" className="footer-glass-pill px-6 py-3 rounded-full text-muted-foreground font-medium text-xs md:text-sm hover:text-foreground">
                  AI / ML
                </MagneticButton>
                <MagneticButton as="a" href="#web" className="footer-glass-pill px-6 py-3 rounded-full text-muted-foreground font-medium text-xs md:text-sm hover:text-foreground">
                  Web &amp; Open Source
                </MagneticButton>
              </div>
            </div>
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="footer-glass-pill px-6 py-3 rounded-full flex items-center gap-2 order-1 md:order-2 cursor-default">
              ...
              <span className="text-foreground font-black text-xs md:text-sm tracking-normal ml-1">Vedant Pol</span>
            </div>
