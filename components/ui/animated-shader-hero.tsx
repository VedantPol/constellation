...
// Reusable Shader Background Hook (WebGL2 nebula by Matthias Hurrle, @atzedent)
...
void main(){gl_Position=position;}`;
  ...
  class PointerHandler {
    ...
    constructor(element: HTMLCanvasElement, scale: number) {
      this.scale = scale;
      const map = (el: HTMLCanvasElement, scale: number, x: number, y: number) => [x * scale, el.height - y * scale];
  ...
  useEffect(() => {
    ...
    if (reduced) {
      rendererRef.current.render(0); // single static frame
    } else {
      loop(0);
    }
...
function HeroButton({ spec, variant }: { spec: ButtonSpec; variant: "primary" | "secondary" }) {
  ...
  return (
    <button onClick={spec.onClick} className={classes}>
      {spec.text}
    </button>
  );
}

const Hero: React.FC<HeroProps> = ({ trustBadge, headline, subtitle, buttons, id, className = "" }) => {
  const canvasRef = useShaderBackground();

  return (
    <div id={id} className={`relative w-full h-screen overflow-hidden bg-ink ${className}`}>
      <canvas
        ref={canvasRef}
        aria-hidden="true"
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        {trustBadge && (
          <div className="mb-8 animate-fade-in-down">
            <div className="flex items-center gap-2 px-5 py-2.5 bg-white/5 backdrop-blur-md border border-white/10 rounded-full text-sm">
              {trustBadge.icons?.map((icon, index) => (
                <span key={index} className="inline-flex items-center">
                  {icon}
                </span>
              ))}
              <span className="text-slate-200">{trustBadge.text}</span>
            </div>
          </div>
        )}
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          ...
          {buttons && (
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10 animate-fade-in-up animation-delay-800">
              {buttons.primary && <HeroButton spec={buttons.primary} variant="primary" />}
              {buttons.secondary && <HeroButton spec={buttons.secondary} variant="secondary" />}
            </div>
          )}
        </div>
      </div>
